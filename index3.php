...
if (isset($_GET['delete'])) {
  ...
  // Delete associated records from the 'attendance' table
  $deleteAttendanceSql = "DELETE FROM `attendance` WHERE `studentId` = $sno";
  $resultAttendance = mysqli_query($conn, $deleteAttendanceSql);

  if (!$resultAttendance) {
      echo "Couldn't delete associated attendance records: " . mysqli_error($conn);
  }

  // Now you can delete the student record
  ...
  if ($result) {
      ...
      echo "Couldn't delete the record: " . mysqli_error($conn);
  }
}
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
 ...
 $sql="UPDATE `students` SET `Name`='$names',`Username` = '$usernames',`password`='$passwords' WHERE `students`.`sno` = $sno";
$result=mysqli_query($conn,$sql);
if($result){
$update=true;
}
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      . <form action="/phpglab3/index3.php" method="POST">
...
sno=e.target.id.substr(1,);
 if(confirm("are u sure u want to  delete this note")){
  ...
  console.log("yes");
  window.location=`/phpglab3/index3.php?delete=${sno}`;
 }
 else{
  console.log("no");
 }
